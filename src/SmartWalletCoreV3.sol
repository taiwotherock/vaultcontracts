...
contract SmartWalletCoreV3 is EIP712,ReentrancyGuard {
   ...
   function _addGuardian(address user) internal { 
    ...
    function executeTransfer(
        ...
        emit DebugUint("nonce", nonce);
        emit DebugUint("deadline", deadline);
        emit DebugUint("sessionId", sessionId);
        emit DebugString("paymentId", paymentId);
        
        require(block.timestamp <= deadline, "EXPIRED");
        emit DebugUint("block.timestamp", block.timestamp);
        
        require(amount <= maxTxAmount, "TX_AMOUNT_EXCEEDS_LIMIT");
        emit DebugUint("maxTxAmount", maxTxAmount);
        
        require(amount > 0, "zero amount");
        
        require(whitelisted[to], "address not whitelisted");
        emit DebugBool("whitelisted[to]", whitelisted[to]);
        
        require(staffs[msg.sender] || owner == msg.sender,"not authorized");
        emit DebugBool("staffs[msg.sender]", staffs[msg.sender]);
        emit DebugAddress("owner", owner);

        // Track daily transfer
        uint256 dayNumber = block.timestamp / 1 days;
        emit DebugUint("dayNumber", dayNumber);
        emit DebugUint("dailyTransferred[dayNumber]", dailyTransferred[dayNumber]);
        emit DebugUint("dailyLimit", dailyLimit);
        ...
        emit DebugBytes32("paymentIdHash", paymentIdHash);
        ...
        emit DebugBytes32("structHash", structHash);
        ...
        emit DebugAddress("expected signer (msg.sender)", msg.sender);
        ...
        emit DebugUint("current sessionNonce", sessionNonces[sessionId][signer]);
        emit DebugUint("provided nonce", nonce);
