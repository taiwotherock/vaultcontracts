...
contract SmartWalletCoreV10 is EIP712, ReentrancyGuard {

    using ECDSA for bytes32;
    using SafeERC20 for IERC20;

    // ─── Constants ────────────────────────────────────────────────────────────
    uint256 public constant MAX_DAILY_LIMIT       = 100_000_000 * 1e6;
    uint256 public constant MAX_TX_AMOUNT         = 10_000_000  * 1e6;
    ...
    uint256 public staffTranLimit;
    uint256 public staffDailyLimit;
    ...
    // ─── Events ───────────────────────────────────────────────────────────────
    event TransferExecuted(address indexed token, address indexed to, uint256 amount, uint256 nonce, string paymentId,address walletAddress);
    ...
    function setStaffLimit(uint256 _dailyLimit, uint256 _tranLimit) external onlyOwner {
        require(_dailyLimit > 0 && _dailyLimit <= MAX_DAILY_LIMIT, "OUT_OF_RANGE");
        require(_tranLimit > 0 && _tranLimit <= MAX_TX_AMOUNT, "OUT_OF_RANGE");
        emit StaffLimitChanged(dailyLimit, _dailyLimit, staffTranLimit, _tranLimit);
        staffDailyLimit = _dailyLimit;
        staffTranLimit = _tranLimit;
    }
    ...
    function _validateSignature(
        ...
        require(transactionsByRef[paymentIdHash].sender == address(0), "PAYMENT_ID_EXISTS");
    ...
    function executePayment(
       ...
       /*transactionsByRef[refNo] = TransactionLog({
        ...
        senderRefs[walletAddress].push(refNo);
       
        nonces[walletAddress]++;
        emit PaymentExecuted(token, walletAddress, signer, amount, nonce, paymentId);
    }
    ...
    function executeTransfer(
        ...
        require(staffs[walletAddress] || walletAddress == owner, "WALLET_ADDR_NOT_AUTHORIZED");
        
         // Daily limit
        uint256 dayNumber = block.timestamp / 1 days;
        if(walletAddress == owner)
        {
            require(amount <= maxTxAmount,"Transaction Limit Exceeded");
            require(dailyTransferred[dayNumber] + amount <= dailyLimit,"DAILY_LIMIT_EXCEEDED");
        }
        else
        {
            require(amount <= staffTranLimit,"Transaction Limit Exceeded");
            require(dailyTransferred[dayNumber] + amount <= staffDailyLimit,"DAILY_LIMIT_EXCEEDED");
        }
       ...
       nonces[walletAddress]++;
        ...
        emit TransferExecuted(token, to, amount, nonce, paymentId,walletAddress);
    }
    ...
    function getLimits()
        ...
        returns (
            uint256 _dailyLimit,
            uint256 _maxTxAmount,
            uint256 _dailyStaffLimit,
            uint256 _staffTranLimit
        )
    {
        return (
            dailyLimit,
