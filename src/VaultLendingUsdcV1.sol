...
import "./VaultStorageUsdc.sol";
import "./SimpleERC20.sol";
import "./SimpleOwnable.sol";
import "./IBNPLAttestationOracle.sol";
//import "@openzeppelin/contracts/token/ERC20/ERC20.sol";
//import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
//import "@openzeppelin/contracts/access/Ownable.sol";

contract VaultLendingUsdcV1 is VaultStorageUsdc, SimpleERC20, SimpleOwnable {
    ...
    event MinimumCreditScoreChanged(uint256 indexed amount);
    ...
    constructor(address _accessControl,  address _platformTreasury, address _stable, address _attestationOracle,
    ...
    function setMinCreditScore(uint256 minCreditScore) external onlyAdmin {
        _minCreditScore = minCreditScore;
        emit MinimumCreditScoreChanged(minCreditScore);
    }
    ...
    function deposit() external payable  returns (uint256 sharesMinted) {
        ...
        vault[msg.sender] += amount;       // user’s deposited balance
    ...
    function createLoan(bytes32 ref,address merchant, uint256 principal,
     ...
     external onlyCreditOfficer {
        ...
        require(vault[borrower] >= depositAmount, "Insufficient vault balance");
        ...
        // Lock borrower credit in oracle
        ...
        merchantFund[merchant] += merchantSettledFund;
        totalMerchantFund += merchantSettledFund;
    ...
    // Repay loan using ERC-2612 permit
    function repayLoanWithPermit(
        ...
        attestationOracle.decreaseUsedCredit(L.borrower, amount);
        ...
        L.totalPaid += amount;
        L.repaidFee += platformFee + lenderFee;

        if (L.outstanding == 0) {
            L.status = LoanStatus.Paid;
            _removeLoanFromBorrower(loanRef);
            emit LoanClosed(loanRef, msg.sender,lenderFee,platformFee);
        }
        else
          emit LoanRepaid(loanRef, msg.sender, amount,lenderFee,platformFee);
    ...
    function getMerchantFund(address merchant) external view onlyWhitelisted(msg.sender) returns (uint256) {
        return merchantFund[merchant];
    }
     
     function getTotalMerchantFund() external view onlyAuthorized() returns (uint256)
     { 
        return totalMerchantFund;
     }
    ...
    function getVaultBalance(address borrower) external view returns (uint256) {
