...
import "./FiatStablecoinAMMViewV1.sol";

contract FiatStablecoinAMMV1 is Ownable, ReentrancyGuard, EIP712 {
    ...
    /// @dev UTC day number → stats for that day (DailyStats defined in FiatStablecoinAMMViewV1)
    ...
    function _checkSwapSizeView(uint256 amount, uint256 pool) internal view {
        if (!swapCheckEnabled) {
            return;
        }
   ...
   function _calcFee(uint256 amount) internal view
    ...
    // These functions provide state access for FiatStablecoinAMMViewV1
    // -------------------------------------------------------
    ...
    {
        require(poolDeposits(USDToken()) > 0 && poolDeposits(cNGNToken()) > 0, "pool empty");

        uint256 gross = ngnAmount.mulDiv(USD_SCALE_VAL(), buyRate());
        (uint256 totalFee, uint256 lpFee, uint256 platformFee) = _calcFeeView(gross);
        ...
        require(usdPool > minLiquidity(USDToken()), "USD pool too small");
        require(usdPool >= gross, "insufficient USD pool");
        require(net <= (usdPool - totalFee).mulDiv(MAX_WITHDRAW_BPS_CONST(), FEE_DENOM_CONST()), "pool cap max swap reached");
        require(usdPool >= minLiquidity(USDToken()) + gross, "insufficient buffer");

        _checkSwapSizeView(gross, usdPool);

        return (net, totalFee, buyRate());
    }
        ...
        )
    {
        ...
        uint256 gross = usdAmount.mulDiv(sellRate(), USD_SCALE_VAL());
        require(gross >= MIN_FEE_CONST(), "fee too small");
        (uint256 totalFee, uint256 lpFee, uint256 platformFee) = _calcFeeView(gross);
        ...
        require(ngnPool > minLiquidity(cNGNToken()), "NGN pool too small");
        require(ngnPool >= gross, "insufficient NGN pool");
        require(net <= (ngnPool - totalFee).mulDiv(MAX_WITHDRAW_BPS_CONST(), FEE_DENOM_CONST()), "pool cap");
        require(ngnPool >= minLiquidity(cNGNToken()) + gross, "insufficient buffer");

        _checkSwapSizeView(gross, ngnPool);

        return (net, totalFee, sellRate());
    }

    function cNGNToken() internal view override returns (address) {
        return address(cNGN);
    }

    function USDToken() internal view override returns (address) {
        return address(USD);
    }

    function feeVaultAddr() internal view override returns (address) {
        return address(feeVault);
    }

    function poolDeposits(address token) internal view override returns (uint256) {
        return poolDeposits[token];
    }

    function minLiquidity(address token) internal view override returns (uint256) {
        return minLiquidity[token];
    }

    function buyRate() internal view override returns (uint256) {
        return buyRate;
    }

    function sellRate() internal view override returns (uint256) {
        return sellRate;
    }

    function midPrice() internal view override returns (uint256) {
        return midPrice;
    }

    function lastPriceUpdate() internal view override returns (uint256) {
        return lastPriceUpdate;
    }

    function maxPriceAge() internal view override returns (uint256) {
        return maxPriceAge;
    }

    function pendingPrice() internal view override returns (uint256) {
        return pendingPrice;
    }

    function priceActiveAt() internal view override returns (uint256) {
        return priceActiveAt;
    }

    function priceUpdateDelay() internal view override returns (uint256) {
        return priceUpdateDelay;
    }

    function maxEpochVolumeBps() internal view override returns (uint256) {
        return maxEpochVolumeBps;
    }

    function swapsPaused() internal view override returns (bool) {
        return swapsPaused;
    }

    function lastSafePrice() internal view override returns (uint256) {
        return lastSafePrice;
    }

    function pauseTimestamp() internal view override returns (uint256) {
        return pauseTimestamp;
    }

    function PAUSE_COOLDOWN_CONST() internal view override returns (uint256) {
        return PAUSE_COOLDOWN;
    }

    function MAX_LIQUIDITY_DRAIN_BPS_CONST() internal view override returns (uint256) {
        return MAX_LIQUIDITY_DRAIN_BPS;
    }

    function maxSwapBps() internal view override returns (uint256) {
        return maxSwapBps;
    }

    function MIN_TRADE_USD() internal view override returns (uint256) {
        return MIN_TRADE_USD;
    }

    function MIN_TRADE_CNGN() internal view override returns (uint256) {
        return MIN_TRADE_CNGN;
    }

    function swapFeeBps() internal view override returns (uint256) {
        return swapFeeBps;
    }

    function FEE_DENOM_CONST() internal view override returns (uint256) {
        return FEE_DENOM;
    }

    function lpShareBps() internal view override returns (uint256) {
        return lpShareBps;
    }

    function USD_SCALE_VAL() internal view override returns (uint256) {
        return USD_SCALE;
    }

    function epochVolumeUSD(address user) internal view override returns (uint256) {
        return epochVolumeUSD[user];
    }

    function epochVolumeCNGN(address user) internal view override returns (uint256) {
        return epochVolumeCNGN[user];
    }

    function epochStart(address user) internal view override returns (uint256) {
        return epochStart[user];
    }

    function epochDuration() internal view override returns (uint256) {
        return epochDuration;
    }

    function liquidityHighWaterMark(address token) internal view override returns (uint256) {
        return liquidityHighWaterMark[token];
    }

    function liquidityWindowStart(address token) internal view override returns (uint256) {
        return liquidityWindowStart[token];
    }

    function drainObservationWindow() internal view override returns (uint256) {
        return drainObservationWindow;
    }

    function MIN_FEE_CONST() internal view override returns (uint256) {
        return MIN_FEE;
    }

    function MAX_WITHDRAW_BPS_CONST() internal view override returns (uint256) {
        return MAX_WITHDRAW_BPS;
    }

    function dailyStats(uint256 dayId) internal view override returns (DailyStats memory) {
        return dailyStats[dayId];
    }

    function today() internal view override returns (uint256) {
        return block.timestamp / 86400;
    }

    function totalSwapVolumeNGN() internal view override returns (uint256) {
        return totalSwapVolumeNGN;
    }

    function totalSwapVolumeUSD() internal view override returns (uint256) {
        return totalSwapVolumeUSD;
    }

    function totalSwapCountNGN() internal view override returns (uint256) {
        return totalSwapCountNGN;
    }

    function totalSwapCountUSD() internal view override returns (uint256) {
        return totalSwapCountUSD;
    }

    function totalLpFeeUSD() internal view override returns (uint256) {
        return totalLpFeeUSD;
    }

    function totalLpFeeCNGN() internal view override returns (uint256) {
        return totalLpFeeCNGN;
    }

    function totalPlatformFeeUSD() internal view override returns (uint256) {
        return totalPlatformFeeUSD;
    }

    function totalPlatformFeeCNGN() internal view override returns (uint256) {
        return totalPlatformFeeCNGN;
    }

    function pendingClaim(address lp, address token) internal view override returns (uint256) {
        return feeVault.pendingClaim(lp, token);
    }

    function shares(address token, address lp) internal view override returns (uint256) {
        return feeVault.shares(token, lp);
    }

    function totalShares(address token) internal view override returns (uint256) {
        return feeVault.totalShares(token);
    }

    function _calcFeeView(uint256 amount) internal view override returns (uint256 total, uint256 lpFee, uint256 platformFee) {
        return _calcFee(amount);
    }

    function _checkSwapSizeView(uint256 amount, uint256 pool) internal view override {
        uint256 cap = Math.mulDiv(pool, maxSwapBps, FEE_DENOM);
        require(amount <= cap, "swap too large: exceeds maxSwapBps of pool");
    }
}
